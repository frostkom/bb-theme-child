	...
	function cron_base()
	{
		...
		if($obj_cron->is_running == false)
		{
			...
			if($this->get_woocommerce_custom_orders_table_enabled() == 'yes')
			{
				...
				$result = $wpdb->get_results($wpdb->prepare("SELECT ".$wpdb->prefix."wc_orders.id FROM ".$wpdb->prefix."wc_orders LEFT JOIN ".$wpdb->prefix."wc_orders_meta ON ".$wpdb->prefix."wc_orders.id = ".$wpdb->prefix."wc_orders_meta.order_id AND meta_key = %s WHERE type = %s AND (status = %s OR status = %s) AND date_updated_gmt < DATE_SUB(NOW(), INTERVAL 15 MINUTE) AND date_updated_gmt > DATE_SUB(NOW(), INTERVAL 36 HOUR) AND meta_value IS NULL ORDER BY id ASC LIMIT 0, 1", $this->meta_prefix.'optima_post_data', 'shop_order', 'wc-processing', 'wc-cancelled'));
			}
